...
export function calculateWaterProfile(
  ...
  additions.forEach((addition) => {
    ...
    const ppmPerGramPerLiter = saltComposition[addition.ionType] ?? 0
    if (ppmPerGramPerLiter === 0) return

    const treatedVolume = addition.volumeL > 0 ? addition.volumeL : 1

    const contributionPpm =
      (addition.amountG / treatedVolume) * ppmPerGramPerLiter

    result[addition.ionType] = Math.max(
      0,
      (result[addition.ionType] ?? 0) + contributionPpm,
    )
  })
