...
export function useRecipeCalculations(
  ...
  return useMemo(() => {
    ...
    const fermentables: Fermentable[] = data.fermentables.map((f) => {
      const potential = f.ingredient.potential ?? f.ingredient.ppg ?? 0
    ...
    const fermentablesColor: FermentableColor[] = data.fermentables.map(
      (f) => ({
        color: f.ingredient.color ?? f.ingredient.colorLovibond ?? 0,
    ...
    // Calculate gravity
    const og = calculateOG(fermentables, data.batchSizeL, data.efficiency / 100)
